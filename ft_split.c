...
static long	get_part_length(char const *s, char c)
{
	long	length;
...
static int	create_parts(char ***spltted, char const *s, char c)
{
	long	pos;
	long	part;
	long	pos_part;
...
static void	clear_memory(char ***spltted, long parts_created)
{
	long	part;
...
char	**ft_split(char const *s, char c)
{
	...
	if (parts_created != 0)
	{
		clear_memory(&spltted, parts_created);
		return (NULL);
	}
	return (spltted);
}
